...
const walletTypes = [
    ...
    },
    {
        ...
        label: 'Uang Tunai',
        icon: Coins,
        desc: 'Dompet Fisik, Amplop Cash',
    },
    {
        type: 'investment',
        label: 'Investasi',
        icon: TrendingUp,
        desc: 'Bibit, Bareksa, Crypto, Saham',
    },
    {
        type: 'credit_card',
        label: 'Kartu Kredit',
        icon: CreditCard,
        desc: 'CC Paylater & Tagihan',
    },
];
...
function submitWallet() {
    form.transform((data) => ({
        ...data,
        balance:
            data.balance === '' || data.balance === null
                ? 0
                : Number(data.balance),
    })).post(walletStore.url(), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset();
...
<template>
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-zinc-500">
                    Kelola rekening bank, e-wallet, dan kas bersama pasangan
                </p>
            </div>
        ...
        <div
            class="relative overflow-hidden rounded-3xl border border-zinc-200/80 bg-gradient-to-br from-indigo-900 via-zinc-900 to-zinc-950 p-6 text-white shadow-xl dark:border-zinc-800"
        >
            <div
                class="absolute -top-10 -right-10 h-32 w-32 rounded-full bg-rose-500/20 blur-2xl"
            />
            <div class="relative z-10 space-y-3">
                <span
                    class="text-xs font-medium tracking-wider text-zinc-400 uppercase"
                    >Total Saldo Gabungan (Net Worth)</span
                >
                ...
                <div
                    class="grid grid-cols-3 gap-2 border-t border-white/10 pt-2 text-xs"
                >
                    <div>
                        ...
                        <p class="font-bold">
                            Rp
                            {{ Number(user_net_worth).toLocaleString('id-ID') }}
                        </p>
                    </div>
                    <div>
                        ...
                        <p class="font-bold">
                            Rp
                            {{
                                Number(partner_net_worth).toLocaleString(
                                    'id-ID',
                                )
                            }}
                        </p>
                    ...
                    <div>
                        ...
                        <p class="font-bold">
                            Rp
                            {{
                                Number(joint_net_worth).toLocaleString('id-ID')
                            }}
                        </p>
                    </div>
                </div>
        ...
        <section class="space-y-3">
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
                ...
                >
                    Belum ada Kas Bersama. Buat dompet tipe "Kas Bersama" untuk
                    patungan kencan dan bayar impian berdua!
                </div>
            </div>
        </section>
            ...
            >
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-zinc-500">
                                Pilih bank, e-wallet, atau kas bersama
                            </p>
                        </div>
                    </div>
                    <button
                ...
                >
                    <div class="flex items-center justify-between">
                        ...
                        <Landmark
                            v-if="form.wallet_type === 'bank'"
                            class="h-5 w-5 opacity-80"
                        />
                        <Smartphone
                            v-else-if="form.wallet_type === 'ewallet'"
                            class="h-5 w-5 opacity-80"
                        />
                        <Coins
                            v-else-if="form.wallet_type === 'cash'"
                            class="h-5 w-5 opacity-80"
                        />
                        <TrendingUp
                            v-else-if="form.wallet_type === 'investment'"
                            class="h-5 w-5 opacity-80"
                        />
                    ...
                    <div class="mt-4">
                        <p
                            class="font-mono text-[11px] tracking-widest uppercase opacity-80"
                        >
                            {{
                                form.account_number
                                    ? form.account_number
                        ...
                        <h3 class="mt-0.5 text-lg font-black tracking-tight">
                            {{ form.name ? form.name : 'Nama Rekening / Bank' }}
                        </h3>
                    </div>

                    <div
                        class="mt-3 flex items-baseline justify-between border-t border-white/20 pt-2 text-xs"
                    >
                ...
                <div class="space-y-1.5">
                    ...
                    <div
                        class="flex max-h-24 flex-wrap gap-1.5 overflow-y-auto pr-1"
                    >
                        <button
                            ...
                            :class="[
                                form.name === preset.name
                                    ? 'border-indigo-600 bg-indigo-50 text-indigo-700 ring-1 ring-indigo-500/20 dark:border-indigo-400 dark:bg-indigo-950/50 dark:text-indigo-300'
                ...
                <form @submit.prevent="submitWallet" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <button
                                ...
                                :class="
                                    form.type === 'personal'
                                        ? 'border-indigo-600 bg-indigo-600/10 text-indigo-600 shadow-sm dark:border-indigo-400 dark:bg-indigo-950/40 dark:text-indigo-400'
                            ...
                            <button
                                ...
                                :class="
                                    form.type === 'joint'
                                        ? 'border-emerald-600 bg-emerald-600/10 text-emerald-600 shadow-sm dark:border-emerald-400 dark:bg-emerald-950/40 dark:text-emerald-400'
                                        : 'border-zinc-200 text-zinc-600 dark:border-zinc-800 dark:text-zinc-400'
                                "
                            >
                    ...
                    <div class="space-y-1">
                        <label
                            class="block text-xs font-semibold text-zinc-700 dark:text-zinc-300"
                            >Nomor Rekening / No. HP E-Wallet (Opsional)</label
                        >
                        <input
            ...
            >
                ...
                >
                    <div class="flex items-center justify-between">
                        ...
                        <Landmark
                            v-if="editForm.wallet_type === 'bank'"
                            class="h-5 w-5 opacity-80"
                        />
                        <Smartphone
                            v-else-if="editForm.wallet_type === 'ewallet'"
                            class="h-5 w-5 opacity-80"
                        />
                        <Coins
                            v-else-if="editForm.wallet_type === 'cash'"
                            class="h-5 w-5 opacity-80"
                        />
                        <TrendingUp
                            v-else-if="editForm.wallet_type === 'investment'"
                            class="h-5 w-5 opacity-80"
                        />
                        <CreditCard v-else class="h-5 w-5 opacity-80" />
                    </div>
                    ...
                    >
                        ...
                        <span class="text-base font-extrabold">
                            Rp
                            {{
                                Number(editForm.balance || 0).toLocaleString(
                                    'id-ID',
                                )
                            }}
                        </span>
