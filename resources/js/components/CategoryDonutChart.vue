...
import { PieChart, Users, User as UserIcon } from '@lucide/vue';
...
function toggleSelectedCategory(category: CategorySpending): void {
    hoveredCategory.value =
        hoveredCategory.value?.id === category.id ? null : category;
}
</script>
    ...
    >
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
                <div
                    class="flex h-8 w-8 items-center justify-center rounded-xl bg-rose-500/10 text-rose-600 dark:bg-rose-500/20 dark:text-rose-400"
                >
        ...
        <div
            ...
            class="flex flex-col items-center gap-5 pt-2 sm:flex-row"
        >
            <!-- SVG Donut Chart -->
            <div class="relative flex shrink-0 items-center justify-center">
                ...
                >
                    ...
                    <circle
                        ...
                        class="hover:stroke-width-14 cursor-pointer transition-all duration-300"
                ...
                <div
                    class="pointer-events-none absolute flex flex-col items-center justify-center px-2 text-center"
                >
                    <span class="text-[10px] font-medium text-zinc-400">
                        {{ hoveredCategory ? hoveredCategory.name : 'Total' }}
                    </span>
                    <span
                        class="text-xs font-black text-zinc-900 dark:text-zinc-100"
                    >
                        {{
                            hoveredCategory
            ...
            <!-- Category List Chips -->
            <div class="max-h-44 w-full space-y-2 overflow-y-auto pr-1">
                <button
                    ...
                    @mouseleave="hoveredCategory = null"
                    @focus="hoveredCategory = cat"
                    @blur="hoveredCategory = null"
                    @click="toggleSelectedCategory(cat)"
                    :aria-pressed="hoveredCategory?.id === cat.id"
                    class="flex min-h-11 w-full cursor-pointer items-center justify-between rounded-2xl border p-2 text-left text-xs transition-all focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500"
                    :class="[
                        hoveredCategory?.id === cat.id
                            ? 'border-indigo-500 bg-indigo-50/50 ring-1 ring-indigo-500/20 dark:bg-indigo-950/30'
                ...
                >
                    <div class="flex items-center gap-2">
                        ...
                        />
                        <span
                            class="max-w-[120px] truncate font-bold text-zinc-800 dark:text-zinc-200"
                        >
                            {{ cat.name }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            class="rounded-md bg-zinc-200/60 px-1.5 py-0.5 text-[10px] font-bold text-zinc-700 dark:bg-zinc-700 dark:text-zinc-300"
                        >
                            {{ cat.percentage }}%
                        </span>
                    </div>
        ...
        >
            ...
            <div
                class="flex h-2 w-full overflow-hidden rounded-full bg-zinc-100 dark:bg-zinc-800"
            >
                <div
